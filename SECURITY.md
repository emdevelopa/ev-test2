...
**How we stop it:** `AresProposalManager` reads the proposer's vote balance from the governance token before storing the proposal. If the balance is below the `proposalThreshold`, it reverts with `BelowProposalThreshold()`. No token balance, no proposal.
